"""
BAKOME Gold Scalper Original — 完整 ICT strategy（原始版移植）
===========================================================
...
- H4 EMA200 趋-trend方向定 Bias
- 仅 Silver Bullet session交易（伦敦 8-9、纽约 15-16，MT4 时区 UTC+3）
- 三重confirm机制：Liquidity Sweep / FVG / Order Block 至少 2 成立才Entry
- ATR 动态risk：SL=2.0xATR, TP=3.0xATR, PnL平衡 + trailing止损
...
STRATEGY_CHANGELOG = [
    {"version": "v1_original", "magic": 880303, "date": "2026-08-02",
     "desc": "初始移植：BAKOME-Hub/BAKOMEPythonGoldScalper 完整 ICT strategy"},
]


class BAKOMEGoldScalperOriginalStrategy(BaseStrategy):
    """BAKOME Gold Scalper Original — 完整 ICT strategy（FVG + OB + Liquidity Sweep + Silver Bullet）"""
    ...
    MAX_POSITIONS = 2
    MAX_DAILY_TRADES = 10
    SIGNAL_MIN_CONFIRMATIONS = 2  # 三重confirm至少需要 2 
    ...
    def _is_in_silver_bullet(self) -> bool:
        """checkcurrent是否在 Silver Bullet session。回测时使用 _backtest_time 覆盖。"""
    ...
    def _is_in_trading_session(self) -> bool:
        """checkcurrent是否在允许 交易session内。回测时使用 _backtest_time 覆盖。"""
        now = getattr(self, '_backtest_time', None) or datetime.now()
        h = now.hour
    ...
    def _detect_order_block(self) -> Optional[OrderType]:
        """检测 Order Block（强-trend突破前 reverse K 线）。"""
        candles = self.candles
        if len(candles) < 6:
            return None
        n = len(candles)
        ...
        # 多头 OB：2  candles大阳线 → 找之前 阴线作为 OB 区
        ...
        # 空头 OB：2  candles大阴线 → 找之前 阳线作为 OB 区
        if c1.close < c1.open and c2.close < c2.open:
            body1 = abs(c1.close - c1.open)
            body2 = abs(c2.close - c2.open)
            if body2 > avg_body * 1.2 and body1 > avg_body * 1.5:
    ...
    def _detect_liquidity_sweep(self) -> Optional[str]:
        """检测 Liquidity Sweep（流动性扫描：价格突破近期摆动高/低 points后反转）。"""
        candles = self.candles
        if len(candles) < 20:
            return None
        n = len(candles)
        ...
        # 多头 LS：价格突破近期低 points后回到低 points之上
        if swing_lows:
            nearest_low = min(swing_lows)
            if current_close <= nearest_low + tolerance:
                return "BUY"

        # 空头 LS：价格突破近期高 points后回到高 points之下
        if swing_highs:
    ...
    def check_ema20_exit(self, position, bid: float, ask: float) -> bool:
        """ATR trailing止损 + 硬止损（代替原 EA  PnL平衡 + trailing）。"""
        ...
        if is_buy:
            ...
            if self.USE_BREAK_EVEN and not td["break_even_set"] and profit >= atr_val * self.BE_TRIGGER_ATR:
                ...
                return False  # 让engine统一modify SL
            # trailing止损
    ...
    def _verify_entry(signal: dict, tick_price: float, latest: dict) -> bool:
        """defaultverify：tick 价不跑出近期摆动range。"""
        return True