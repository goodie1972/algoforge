...
- 7因子Score: VWAP替代→EMAposition, RSI, MACD, EMA排列, ADX+DI, 成交量, 次级RSI
- 多级RR出场: 1R/2R/3R/4R/5R, TP1命 后移到保本
- K线收盘confirm
...
class ViprasolSniperStrategy(BaseStrategy):
    ...
    def __init__(self, bridge: MT4BridgeBase, magic: int = 0, timeframe: str = ""):
        ...
        # === Entry params ===
        self.score_threshold = 4  # 7因子 至少4
        self.rsi_period = 14
        self.ema_fast = 9
        self.ema_slow = 21
        self.atr_period = 14
        ...
        self.rr_levels = [2, 3, 4, 5]  # 出场级别从 2R start（1R 只触发保本，不close）
        self.breakeven_r = 1.0  # breakeven hit级别（1R）
        self.trail_atr = 1.0  # 移动trailing
    ...
    def check_ema20_exit(self, position, bid: float, ask: float) -> bool:
        ...
        if ticket not in self._trail_data:
            ...
            self._trail_data[ticket] = {
                ...
                "peak_profit": 0.0,
                "breakeven": False,
                # lockEntry时 1R risk距离，RR 出场价位不随后续 ATR 漂移
        ...
        entry = td["entry"]
        # RR 出场用Entry时lock  1R；若Entry时 ATR 缺失则用current ATR 补锁
        ...
        if is_buy:
            ...
            if not td["breakeven"] and current_profit > risk_r * self.breakeven_r:
                ...
                logger.info(f"[{self.name}] BUY breakeven hit ticket={ticket}")
            ...
            if not td["breakeven"] and current_profit > risk_r * self.breakeven_r:
                td["breakeven"] = True
                self.bridge.modify_order(ticket, sl=entry, tp=0)
                logger.info(f"[{self.name}] SELL breakeven hit ticket={ticket}")
