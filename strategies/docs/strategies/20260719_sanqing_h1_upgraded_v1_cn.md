---
name: sanqing_h1_upgraded
magic: 880108
type: 趋势
display: H1 SanQing 升级版
desc: EMA9/21趋势评分 + 高位拦截 + 动态利润回撤止盈
---

**适用周期：** H1

## 评分因子
| # | 因子 | 得分 | 说明 |
|---|---|---|---|
| 1 | 上涨趋势 | +2 | EMA9 > EMA21 |
| 2 | EMA金叉 | +1 | EMA9上穿EMA21 |
| 3 | 回抽EMA9 | +2 | low ≤ EMA9×1.002 且 close > EMA9 |
| 4 | 实体>1ATR | +1 | 实体长度超过1倍ATR |
| 5 | 放量 | +1 | 成交量 > 均量×1.3 |
| 6 | 吞没形态 | +2 | 实体中位数≥1.5且实体/前高≥1.5且实体占K线≥50% |

| # | 因子 | 得分 | 说明 |
|---|---|---|---|
| 1 | 下跌趋势 | +2 | EMA9 < EMA21 |
| 2 | EMA死叉 | +1 | EMA9下穿EMA21 |
| 3 | 回抽EMA9 | +2 | high ≥ EMA9×0.998 且 close < EMA9 |
| 4 | 实体>1ATR | +1 | 实体长度超过1倍ATR |
| 5 | 放量 | +1 | 成交量 > 均量×1.3 |
| 6 | 吞没形态 | +2 | 实体中位数≥1.5且实体/前高≥1.5且实体占K线≥50% |
## 运动员验票
| 方向 | 条件 |
|---|---|
| BUY | 等价格回抽到 ≤ EMA9×1.002 再入场 |
| SELL | 等价格反弹到 ≥ EMA9×0.998 再入场 |
## 出场逻辑（ADX 自适应）
| # | 条件 | 震荡(ADX≤25) | 中等趋势(ADX 25~35) | 强趋势(ADX>35) |
|---|---|---|---|---|
| ① | ADX自适应追踪止损 | 1.5 ATR 回撤 | 2.5 ATR 回撤 | 3.5 ATR 回撤 |
| ② | ADX自适应止盈 | 2.5 ATR | 4.0 ATR | 6.0 ATR |
| ③ | 硬止损（固定） | 1.2 ATR | 1.2 ATR | 1.2 ATR |
| ④ | 利润回撤+DI保护 | 峰值回撤25%，DI对齐(趋势完好)时跳过回撤止盈 |  |  |
| ⑤ | DI反转出场 | 开仓5分钟后: BUY持仓NDI>PDI / SELL持仓PDI>NDI |  |  |
## 参数说明
| 参数 | 取值 | 说明 |
|---|---|---|
| 评分阈值 | 5 分 | ADX > 25 时降为 4 分（降门槛而非禁入） |
| 追踪止损 | 1.5 / 2.5 / 3.5 ×ATR | 震荡(ADX≤25) / 中等(25~35) / 强趋势(>35) |
| 止盈 | 2.5 / 4.0 / 6.0 ×ATR | 按 ADX 三档自适应 |
| 硬止损 | 1.2 ×ATR | 固定，最后防线 |
| 利润回撤最小持仓 | 1800 秒（30 分钟） | 持仓未满 30 分钟不触发利润回撤止盈 |
| 动态强制止盈 | 峰值≤10 回撤 50%；峰值>10 回撤 35% | 仅趋势保护（DI 对齐且 ADX>20）时适用 |
| SteepMA 加分 | MA14 斜率(回看5根) | 方向一致 +1 分 |
## 风控
- 硬止损：1.2×ATR（固定，防大亏）
- 位置门禁：60 根窗口内 price_position>0.82 且偏离 EMA21 >2.5×ATR 禁多；price_position<0.18 且偏离 <−2.5×ATR 禁空（评分清零）
- 利润回撤最小持仓 30 分钟：给趋势发展时间，避免过早被回撤洗出
- 趋势保护 + 动态强制止盈：DI 对齐且 ADX>20 时跳过普通回撤止盈让利润奔跑，仅在回撤超过动态阈值（峰值≤10→50%，>10→35%）时强制止盈
- DI 翻转出场：开仓 5 分钟后检测，需下一根 K 线确认才出场（防频繁翻转）
- 最大持仓：1 单（STRATEGY_POOL 配置）
## 特别规则
- 阈值: **固定 5 分**（评分 3→5，提高入场门槛，减少低质量信号约60%）
- **ADX 门禁**: ADX>25 时入场阈值由 5 降为 4（20→25，降门槛而非禁入）
- **位置门禁**: 60根K线窗口内，price_position > 0.82 且偏离 EMA21 > 2.5×ATR 禁多；price_position < 0.18 且偏离 < −2.5×ATR 禁空
- **硬止损**: 1.2 ATR（1.5→1.2，每笔约36点，防大亏）
- **v10_optimized (2026-08-08)**: 提高评分阈值至5、ADX阈值至25、硬止损收紧至1.2ATR，减少交易量、提升信号质量
- 数据源: 全部指标从 DataFactory TA-Lib 读取