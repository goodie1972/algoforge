"""
M30 波动率均值回归策略 — v1.0
入场: 价格触及布林带上下轨 + ATR扩张确认 + RSI背离
出场: 回归中轨止盈 + ATR动态止损 + 有限 recovery
...
STRATEGY_CHANGELOG = [
    {"version": "v1.0", "magic": 880302, "date": "2026-08-01",
     "desc": "M30波动率均值回归: BB触及+ATR扩张+RSI背离; 中轨止盈+ATR止损+有限recovery"},
]
...
class M30VolReturnStrategy(BaseStrategy):
    ...
    name = "m30_vol_return"
    display = "M30 波动率均值回归 — BB+ATR+RSI背离"
    MAGIC = 880302
    ...
    def __init__(self, bridge: MT4BridgeBase, **kwargs):
        ...
        # Entry params
        self.atr_expansion_threshold = 1.2      # ATR 扩张倍数（当前ATR/均值ATR）
        self.rsi_oversold = 30                   # RSI 超卖
        self.rsi_overbought = 70                 # RSI 超买
        self.bb_deviation = 2.0                  # BB 标准差倍数

        # Exit params
        self.profit_atr_mult = 1.0               # 回归中轨止盈（ATR倍数）
        self.sl_atr_mult = 1.5                   # 止损 ATR 倍数
        self.recovery_volume_mult = 1.0          # recovery 加仓倍数（=不加仓，只是重开同方向）
    ...
    def generate_signal(self):
        ...
        if atr_list and len(atr_list) >= 5 and atr_val:
            atr_ma = sum(atr_list[-5:]) / 5
            atr_expanding = atr_val > atr_ma * self.atr_expansion_threshold

        # BB 宽度扩张（波动率放大）
        bb_wide = False
        if bb_width is not None:
            bb_wide = bb_width > 8.0  # XAUUSD 黄金 BB 宽度 > 8 表示波动率大
