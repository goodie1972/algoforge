"""
M30 RSI + 布林带均值回归 — 7因子Score系统
=========================
- Entry: 7因子Score系统 ≥4直接开, =3需diff≥2
- 7因子: M30趋-trend, BBposition(90%), RSI分层(25/35/65/75), RSI方向(3 candles), DI强度, 成交量, K线形态
- ADX>28趋-trendGateremove（2026-06-30）
- 出场: 保本出场 + profitdrawdowntake profit + ATRtrailing + DIskip(盈利时) + 硬止损
- 双向交易 (Long / Short)
...
]  # 旧版 magic，enginestart时auto接管
STRATEGY_CHANGELOG = [
    {"version": "v1", "magic": 660701, "date": "2026-06-08", "desc": "初始上线：5因子Score≥3，ATR跟踪止损 trail=4.0 hard=3.0"},
    {"version": "v2", "magic": 660702, "date": "2026-06-08", "desc": "修复出场逻辑：区分盈利/loss阶段，新增 peak_profit 跟踪"},
    {"version": "v3", "magic": 660703, "date": "2026-06-09", "desc": "双重take profit：trail=1.0 hard=2.0，新增 profit_drawdown_pct=0.25，新增 indicator_values return"},
    {"version": "v4", "magic": 660704, "date": "2026-06-11", "desc": "RSI分层filter：RSI<20禁空，RSI20-30空头扣1分；新增 tight_exit_mode newsrisk"},
    {"version": "v5", "magic": 660705, "date": "2026-06-12", "desc": "positionGate：60 candlesK线rangebottom10%禁空、top10%禁多"},
    {"version": "v6", "magic": 660706, "date": "2026-06-15", "desc": "趋-trend判断改为M30自身SMA200（替代H1 SMA200），盈利close后同方向30mincooldown"},
    {"version": "v7", "magic": 660707, "date": "2026-06-19", "desc": "趋-trend判断MA14替代SMA200，去掉低volatility因子⑤，4因子Score"},
    {"version": "v8", "magic": 660707, "date": "2026-06-22", "desc": "removetight_exit_mode and RSI短侧filter"},
    {"version": "v9", "magic": 660707, "date": "2026-06-22", "desc": "新增ADX>28趋-trendGate(EMA9/21), RSI方向改3 candles连续confirm"},
    {"version": "v10", "magic": 660707, "date": "2026-06-22", "desc": "新增DItake profit判定: 移动take profit触发时+DI- -DI>10(BUY)/-DI-+DI>10(SELL)则ignoretake profit"},
    {"version": "v11", "magic": 660707, "date": "2026-06-22", "desc": "新增DI强度因子⑤: |DI差|>10 给±1分, 5因子Scorethreshold保持3"},
    {"version": "v12", "magic": 660707, "date": "2026-07-01", "desc": "H1 MA20趋-trendGate替代ADXGate"},
    {"version": "v13", "magic": 660707, "date": "2026-07-01", "desc": "H1Gate放过extreme区：BBupper band10%外或RSI深超买时允许SELL，lower band10%外或深超卖时允许BUY"},
    {"version": "v14_optimized", "magic": 660707, "date": "2026-08-08", "desc": "optimize: score_threshold=5, hard_atr=1.2, trailing_atr=1.5, rsi_oversold=30/deep=20, rsi_overbought=70/deep=80"},
]
...
class M30RSIStrategy(BaseStrategy):
    ...
    def __init__(self, bridge: MT4BridgeBase, magic: int = 0, timeframe: str = ""):
        ...
        self.p_trailing_atr = 1.5   # 回调超过 1.5 ATR 即take profit（原为 1.0）
        self.p_hard_atr = 1.2       # 硬止损 ATR×1.2（原为 2.0）
        # profit_drawdown_pct 继承自 BaseStrategy（default 0.25，由 settings.py 控制）

        # (ADX>28 趋-trendGateremove — 2026-06-30)
        ...
        # 盈利closecooldown：盈利出场后同方向30min内不再Open
        self._last_profit_exit_time: dict[str, float] = {"BUY": 0.0, "SELL": 0.0}
        self._exit_cooldown_seconds: int = 1800  # 30min

    def get_adx_data(self) -> Optional[dict]:
        """return ADX data（含 +DI/-DI），供engineGate使用"""
    ...
    def _detect_candle_pattern(self) -> tuple:
        ...
        try:
            ...
            # 早晨之星/黄昏之星（三 candlesK线confirm，最强反转）
    ...
    def generate_signal(self) -> Optional[OrderType]:
        ...
        # ① M30 trend（原H1 SMA200改为M30自身w期）
        if m30_trend == 'UP':
            long_score += 1; long_detail.append("M30-UP")
        elif m30_trend == 'DOWN':
            short_score += 1; short_detail.append("M30-DN")

        # ② BB position：价格进入上lower band 10% range即触发（原需精确触轨）
        bb_range = bb['upper'] - bb['lower']
        bb_bot_zone = bb['lower'] + bb_range * 0.1   # bottom 10% bound
        bb_top_zone = bb['lower'] + bb_range * 0.9   # top 10% bound
        ...
        #   SHORT: H1=UP 但 price>=BBupper band AND RSI>70 → 放过SELL
        #   LONG:  H1=DOWN 但 price<=BBlower band AND RSI<30 → 放过BUY
        self._load_h1_data()
        ...
        # ── 盈利closecooldown：同方向30min内不Open ──
        if long_score >= self.score_threshold:
            ...
            if remaining > 0:
                ...
                logger.info(f"[{self.name}] BUY cooldown: remaining after profit close {int(remaining)}s")
                long_score = 0
        if short_score >= self.score_threshold:
            ...
            if remaining > 0:
                ...
                logger.info(f"[{self.name}] SELL cooldown: remaining after profit close {int(remaining)}s")
                short_score = 0

        # ── Decision（双层判决：≥4直接开，=3需diff≥2）+ H1Gate拦截 ──
        ...
        # ── Logging ──
        gate_log = ""
        if h1_block_long: gate_log = " [H1NO BUY]"
        elif h1_block_short: gate_log = " [H1NO SELL]"
        detail_parts = []
        if long_detail: detail_parts.append("LONG: " + " ".join(long_detail))
        if short_detail: detail_parts.append("SHORT: " + " ".join(short_detail))
        logger.info(
            f"[{self.name}] Score: {long_score}/{short_score}{gate_log}  {signal_str}  "
    ...
    def get_dynamic_sl_tp(self, direction: OrderType, entry_price: float) -> tuple[float, float]:
        """双层take profit止损：
        ① 固定 SL/TP： candles据 with-trend/逆向/震荡 set不同  ATR x率（MT4 订单级硬bound）
        ② 趋-trendtake profit：由 check_ema20_exit 运行时动态管理（trailing/drawdown）
        """
        atr_val = self.get_indicator("atr_20")
        ...
        if m30_trend == 'NEUTRAL':
            ...
            sl_mult, tp_mult = 2.5, 4.0       # with-trend: SL=2.5ATR  TP=4.0ATR ( and MAsame-dir)
        else:
            sl_mult, tp_mult = 1.0, 2.0       # 逆向: SL=1.0ATR  TP=2.0ATR ( and MAreverse)
    ...
    def check_ema20_exit(self, position, bid: float, ask: float) -> bool:
        ...
        if ticket not in self._trail_data:
            # lockOpen时 趋-trend方向，后续不受 MA14 翻转影响
        ...
        if is_buy:
            ...
            if drawdown > atr_val * trail_mult:
                # DItake profit判定: +DI - -DI > 10 趋-trend仍强, ignoretake profit
                if adx_data and current_profit > 0 and (adx_data["pdi"] - adx_data["ndi"]) > 10:
                    logger.info(f"[{self.name}] BUY DIskiptake profit ticket={ticket} DIs={adx_data['pdi']-adx_data['ndi']:.1f}")
                else:
            ...
            if rally > atr_val * trail_mult:
                ...
                if adx_data and current_profit > 0 and (adx_data["ndi"] - adx_data["pdi"]) > 10:
                    logger.info(f"[{self.name}] SELL DIskiptake profit ticket={ticket} DIs={adx_data['ndi']-adx_data['pdi']:.1f}")
                else:
