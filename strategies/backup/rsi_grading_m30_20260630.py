"""
M30 RSI分级Score — MA14 + BB + ADX>28 趋-trendGate
===========================================
基于回测最优config (m30_final_bt.py section 4):
  - RSI 分级Score: <20→+2, 20-30→+1, >70→+2, 65-70→+1
  - MA14 方向 (±1)
  - BB 触轨 (±1)
  - ADX>28 趋-trendGate: EMA9>EMA21→禁空, EMA9<EMA21→禁多
  - 无 RSI 方向因子，无短侧filter
  - threshold 2, EMA趋-trend感知出场: with2.0/逆1.0
...
class RSIGradingM30Strategy(BaseStrategy):
    """M30 RSI分级Score均值回归 (去RSI方向) + ATR动态出场"""
    ...
    def generate_signal(self) -> Optional[tuple]:
        ...
        # ── Decision (ADX>28 Gate禁reverse; ADX≤28 提高threshold到 3) ──
        signal = None
        ...
        # ADX≤28 无Gate: 需要更高thresholdfilter震荡市假Signal
        ...
        if gate_side and not signal:
            ...
            signal_str += f" (ADX≤{self.adx_threshold} threshold提升至 {effective_threshold})"
