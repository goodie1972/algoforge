"""
M30 Stoch BB 双模策略 T6V1 — 震荡+趋势叠加 (原版，趋势无Stoch交叉)
================================================================
- 震荡模式 (ADX<30 + BB≤1.0): Stoch 9-3-3 金叉/死叉 + EMA21
- 趋势模式 (ADX≥30 + DI主导): EMA21方向对齐 (无Stoch交叉要求)
...
STRATEGY_CHANGELOG = [
    {"version": "v1", "magic": 660901, "date": "2026-06-19", "desc": "初始版本: T6V1 双模 — 震荡Stoch+EMA21, 趋势DI+EMA21, ATR追踪止损"},
]


class M30StochT6V1Strategy(BaseStrategy):
    """M30 Stoch BB 双模策略 T6V1 — 震荡+趋势叠加"""
    ...
    def _calc_stoch(self) -> Optional[dict]:
        """Stoch 9-3-3: return {curr_k, prev_k, curr_d, prev_d, prev2_k, prev2_d}"""
    ...
    def generate_signal(self) -> Optional[OrderType]:
        ...
        # ② 趋势模式: DI 主导 + EMA21 (无 Stoch 交叉)
        if is_trend:
            if (pdi - ndi) > self.di_threshold and close > ma_val:
                long_score += 3
                long_factors.append("TREND-DI-UP")
                long_factors.append(f"DI={pdi - ndi:.0f}")
            elif (ndi - pdi) > self.di_threshold and close < ma_val:
                short_score += 3
