"""
BAKOME Trinity EA Original — 多资产趋势追踪系统（原始版移植）
=============================================================
来源: https://github.com/BAKOME-Hub/BakomeTrinityEA
原始 MQL5 策略 (BakomeTrinityEA.mq5)，完整移植到 Python 系统
- 支持 XAUUSD、GBPUSD、BTCUSD 等多资产
- H1 EMA34 + H4 EMA200 双时间框架趋势判断
- M5 入场执行，EMA34(H1) > EMA200(H4) → BUY，反之 SELL
- ATR 动态风控（SL=2.0xATR, TP=3.0xATR）
- 经济新闻过滤 + 时段过滤
- 盈亏平衡 + 追踪止损管理
...
STRATEGY_CHANGELOG = [
    {"version": "v1_original", "magic": 880304, "date": "2026-08-02",
     "desc": "初始移植：BAKOME-Hub/BakomeTrinityEA 多资产趋势追踪"},
]


class BAKOMETrinityEAOriginalStrategy(BaseStrategy):
    """BAKOME Trinity EA Original — 多资产趋势追踪（H1 EMA34 + H4 EMA200）"""
    ...
    def _check_m5_confirm(self, trend: str) -> bool:
        """
        M5 入场确认：M5 close 位于趋势方向一侧。
        多头趋势时 M5 不应大幅低于 EMA34(H1)，空头趋势时不应大幅高于 EMA34(H1)。
        """
        h1_ema_34 = self.get_indicator("ema_34")
