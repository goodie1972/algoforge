"""
H1 突破趋-trendstrategy — v1.0
Entry: 价格突破过去20 candlesH1 range + ADX>25 confirm趋-trend
出场: EMA20 trailing + ATR 动态止损
...
class H1BreakoutStrategy(BaseStrategy):
    """H1 突破趋-trendstrategy"""

    name = "h1_breakout"
    display = "H1 突破趋-trendstrategy — range突破+ADXconfirm"
    MAGIC = 880301
    ...
    def __init__(self, bridge: MT4BridgeBase, **kwargs):
        ...
        # Entry params
        self.breakout_lookback = 20       # range回溯w期
        self.adx_threshold = 25           # ADX > 25 confirm趋-trend
        self.atr_sl_mult = 1.5            # 止损 ATR x数
        self.atr_tp_mult = 3.0            # take profit ATR x数

        # Exit params (ADX 自适应)
        self.p_trail_chop = 1.0           # 震荡: 窄trailing
        self.p_trail_normal = 1.5         #  等: 正常trailing
        self.p_trail_trend = 2.0          # 强趋-trend: 宽trailing
        self.p_profit_chop = 1.5          # 震荡: 窄take profit
        self.p_profit_normal = 2.5        #  等: 正常take profit
        self.p_profit_trend = 3.5         # 强趋-trend: 宽take profit
        self.p_hard_atr = 1.5             # 硬止损 ATR x数

        # same-dir保护
        self._min_hold_seconds = 600      # minPositions时间 10 min
    ...
    def generate_signal(self):
        ...
        # (4) 价格在 EMA9 下方（短期趋-trend向下）
        if ema9 is not None and close < ema9:
            short_factors.append("EMA9-DOWN")
            sell_score += 1

        # (5) 价格在 EMA20 下方（ 期趋-trend向下）
        if ema20 is not None and close < ema20:
