"""
M30 MFI + 布林带均值回归 (optimize版) — 均值回归strategy
========================================
- Open：MFI 极端值 + BB 触轨（2 candlesK线容差，更精确）
- close：with-trend（另一极端+另一轨）/ 逆-trend（ midline 或 half-width）
- 特 points：更严格  MFI threshold（85/15），更高Signal质量
...
STRATEGY_CHANGELOG = [
    ...
    {"version": "v6_optimized", "magic": 661002, "date": "2026-07-11", "desc": "optimize版：MFIthreshold收紧至85/15，容差降至2 candles，提高Signal质量"},
]


class M30MFIBBOptimized(BaseStrategy):
    """M30 MFI + 布林带均值回归strategy (v6_optimized)"""
    ...
    def __init__(self, bridge: MT4BridgeBase, magic: int = 0, timeframe: str = ""):
        ...
        self.bb_period = 20
        self.bb_std = 2.0
        self.tolerance_bars = 2  # 2 candlesK线容差（更精确）
    ...
    def _check_3bar_condition(self) -> tuple[bool, bool, Optional[dict]]:
        ...
        # SELL: MFI>=85 + price>=BBupper
        sell_signal = bb_touch_upper and mfi_overbought
        # BUY: MFI<=15 + price<=BBlower
    ...
    def check_ema20_exit(self, position: Position, bid: float, ask: float) -> bool:
        """
        v6_optimized close逻辑（ and  v5 相同）：
        - trend exit：MFI另一极端 + 碰另一轨（tolerance_bars  candles容差）
        ...
        # ── ① trend exit：MFI另一极端 + 另一轨（tolerance_bars  candles容差检测） ──
        buy_signal, sell_signal, _ = self._check_3bar_condition()
        if is_buy and sell_signal:
            # BUYPositions → 出现SELLSignal（MFI<=15+BBlower band）
        ...
        if not is_buy and buy_signal:
            # SELLPositions → 出现BUYSignal（MFI>=85+BBupper band）
    ...
    def _verify_entry(signal: dict, tick_price: float, latest: dict) -> bool:
        """
        v6_optimized verify：容差检测由 generate_signal confirm，这里只verifySignal有效性。
        更宽松 verify：价格容忍度放宽至±1.5%，MFIthreshold匹配收紧后 strategyparam。
        - SELL: 价格还在BBupper band±1.5%以内，MFI未跌到15以下
        - BUY:  价格还在BBlower band±1.5%以内，MFI未涨到85以上
        """
