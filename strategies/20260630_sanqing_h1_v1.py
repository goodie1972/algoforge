"""
SanQing EA — H1 实盘strategy
======================
- EMA9/21 趋-trend + ATR14 Score系统
- Scorethreshold: ADX>20 趋-trend =4, ADX≤20=3, 触发 BUY/SELL
- ATR动态trailing止损出场
...
STRATEGY_CHANGELOG = [
    {"version": "v1", "magic": 880101, "date": "2026-06-08", "desc": "初始上线：6因子Score≥5，ATR跟踪止损 trail=4.0 hard=2.5"},
    {"version": "v2", "magic": 880102, "date": "2026-06-08", "desc": "修复出场逻辑：区分盈利/loss阶段，新增 peak_profit 跟踪"},
    {"version": "v3", "magic": 880103, "date": "2026-06-09", "desc": "双重take profit：trail=1.0 hard=2.0，新增 profit_drawdown_pct=0.25，新增 indicator_values return"},
    {"version": "v4", "magic": 880104, "date": "2026-06-11", "desc": "新增 tight_exit_mode newsrisk"},
    {"version": "v5", "magic": 880105, "date": "2026-06-11", "desc": "positionGate：60 candlesK线rangebottom10%禁空、top10%禁多"},
    {"version": "v6", "magic": 880106, "date": "2026-06-12", "desc": "自适应drawdowntake profit：微利单profit_drawdown按peak_profit占比ATR动态放松至50%/40%"},
    {"version": "v6r", "magic": 880106, "date": "2026-06-21", "desc": "回退v6纯with趋-trend逻辑，去掉逆-trend因子；with趋-trend出场加宽至trail=2.5 hard=4.0"},
    {"version": "v7", "magic": 880107, "date": "2026-06-22", "desc": "ADX>25 趋-trend threshold从5降到4；ADX≤25保持threshold5"},
]


class SanQingH1Strategy(BaseStrategy):
    """SanQing EA — H1 EMA9/21 + ATR14 Score系统"""
    ...
    def __init__(self, bridge: MT4BridgeBase, magic: int = 0, timeframe: str = ""):
        ...
        # ADX>20 趋-trend threshold=4；ADX≤20 保持threshold=3
        self.adx_threshold = 20

        # Exit params — 双重take profit：profitdrawdown25% + ATR移动take profit + 硬止损
        self.p_trailing_atr = 1.0   # 回调超过 1 ATR 即take profit（原为 4.0）
    ...
    def generate_signal(self) -> Optional[OrderType]:
        ...
        # ── Position gate: 极端position不做逆-trend交易 ──
        ...
        if price_position < 0.10 and sell_score >= effective_threshold:
            ...
            logger.info(f"[{self.name}] Position Gate: price at range bottom {price_position:.1%}, block SELL (orig score={sell_score})")
            sell_score = 0
        elif price_position > 0.90 and buy_score >= effective_threshold:
            long_factors.append("TOP-GATE")
            logger.info(f"[{self.name}] Position Gate: price at range top {price_position:.1%}, block BUY (orig score={buy_score})")
            buy_score = 0
    ...
    def check_ema20_exit(self, position, bid: float, ask: float) -> bool:
        ...
        if ticket not in self._trail_data:
            # lockOpen时 趋-trend方向，后续不受 EMA9/21 翻转影响
        ...
        if is_buy:
            ...
            # 移动take profit：从最高 points回落（不论PnL）
            ...
            # 移动take profit：从最低 pointsrebound（不论PnL）
            rally = ask - td["lowest"]
