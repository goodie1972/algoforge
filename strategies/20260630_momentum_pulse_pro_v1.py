"""
Momentum Pulse PRO — 7维度多因子Score + 三层TP出场
====================================================
来源: TradingView Momentum Pulse PRO (自适应动量复合 + 多维度共识)
- AMC: RSI+MACD+ROC 合成动量分
- 7维度Score: AMC强度, Signal对齐, RSI区域, 多w期对齐, 成交量confirm, 市场状态, 无衰竭
- 三层TP: TP1=1.5ATR(50%) → TP2=3.0ATR(30%) → 剩余移动trailing
...
class MomentumPulseProStrategy(BaseStrategy):
    ...
    def __init__(self, bridge: MT4BridgeBase, magic: int = 0, timeframe: str = ""):
        ...
        # === Entry params ===
        self.score_threshold = 6  # 7维度 至少6
        self.rsi_period = 14
        self.macd_fast = 12
        self.macd_slow = 26
        self.macd_signal = 9
        ...
        self.trail_atr = 1.5 # 移动trailing

        # 时间止损( candlesK线)
        self.max_bars = 40  # 日内交易default40 candles
    ...
    def _calc_amc(self, closes: list[float]) -> Optional[float]:
        ...
        try:
            import talib
            import numpy as np
            roc_arr = talib.ROC(np.array(closes, dtype=float), timeperiod=10)
            roc = roc_arr[-1] if roc_arr is not None and len(roc_arr) > 0 and roc_arr[-1] == roc_arr[-1] else None
        except Exception:
            roc = None
    ...
    def generate_signal(self) -> Optional[tuple]:
        ...
        # ④ 多w期对齐: H1方向（用 DataFactory 缓存避免阻塞主循环）
        try:
    ...
    def get_dynamic_sl_tp(self, direction: OrderType, entry_price: float) -> tuple[float, float]:
        """初始SL/TP: SL=1.5ATR, TP=TP1(1.5ATR) —  一层目标"""
        atr_val = self.get_indicator("atr")
    ...
    def check_ema20_exit(self, position, bid: float, ask: float) -> bool:
        """三层TP出场: TP1→50%/ TP2→30%/ 剩余移动trailing"""
        ...
        if is_buy:
            ...
            if tp_hit < 1 and current_profit > atr_val * self.tp1_atr:
                self._tp_hit[ticket] = 1
                logger.info(f"[{self.name}] SELL TP1 hit  ticket={ticket} profit=${current_profit:.2f}")
                td["partial_closed"] = True
            if tp_hit < 2 and current_profit > atr_val * self.tp2_atr:
                self._tp_hit[ticket] = 2
                logger.info(f"[{self.name}] SELL TP2 hit  ticket={ticket} profit=${current_profit:.2f}")
            if current_profit > atr_val * self.tp3_atr:
                self._last_exit_detail = {"exit_type": "tp3_hit", "profit": round(current_profit, 2)}
                del self._trail_data[ticket]
                return True
