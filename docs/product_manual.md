...
- **三轨架构** — DataFactory(独立线程拉K线+TA-Lib指标) + 策略员(评分出门票) + Athlete(tick验证开仓)
...
- **信号全生命周期管理** — 从信号生成→开仓→废票→平仓的全链路追踪
...
- **策略颜色自动分配** — 策略名哈希定色相，同色系派生_optimized/_original变体
- **策略版本管理** — Magic Number 标准化(PP+NN+VV)，Changelog 持久化到数据库
...
### 14.1 状态监控 (`tools/status_monitor.py`)

独立进程，每 **5 分钟**通过 REST API 检查一次。

**检查项目：**
1. **引擎状态** — 调用 `/api/engine/status`，异常则自动重启
2. **桥接连接** — 断连等待10秒后未恢复则重启引擎
3. **数据工厂缓存** — 检查指标数据是否就绪
4. **信号/纸面CSV** — 检查30分钟内是否有新数据
5. **主循环异常** — 捕获ERROR日志中的异常并自动重启

### 14.2 旧版巡检 (`monitor/patrol_daemon.py`)

独立进程，每 30 秒通过 REST API 巡检一次。
...
纸面测试用于评估策略信号质量，**只记录不真开仓**。每个信号都会模拟入场，按策略专属规则模拟出场。

### 18.2 组件

| 组件 | 文件 | 说明 |
|------|------|------|
| 信号记录 | `tools/signal_analysis_recorder.py` | 每30秒轮询，记录信号+完整指标快照 |
| 纸面交易 | `tools/paper_trader.py` | 模拟入场+出场，生成交易记录表 |
| 周分析 | `tools/weekly_analysis.py` | 按策略/方向/出场原因生成汇总报告 |
| 状态监控 | `tools/status_monitor.py` | 每5分钟检查引擎/桥接，异常自动重启 |

### 18.3 数据输出

| 文件 | 说明 |
|------|------|
| `logs/paper_trades.csv` | 所有模拟交易记录（编号/时间/策略/方向/入场价/出场价/收益/原因） |
| `logs/signal_analysis.csv` | 信号全指标快照 |
| `papertest/weekN_*/` | 每周归档数据（含Excel报表、CSV、分析报告） |

### 18.4 出场规则

| 策略 | 出场方式 |
|------|---------|
| mfi_bb_m30 系列 | BB中轴/半宽/另一极端(2根容差) |
| 其他策略 | ATR追踪止盈 + 硬止损 + 利润回撤止盈 |
