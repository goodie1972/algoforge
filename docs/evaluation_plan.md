...
*文档版本: v2.0 | 创建日期: 2026-08-02 | 关联: [[portfolio-quantum-king-plan]]*
...
| 项目 | 值 |
|:----|:----|
| 回测时间 | 2026-02-01 ~ 2026-08-01（3个月） |
| 数据源 | 数据库 OHLCV（M15/M30/H1/H4） |
| 初始资金 | $10,000（虚拟） |
| 手数 | 固定 0.01 |
| 出场逻辑 | 通用：硬止损1.5×ATR + EMA21追踪 + BB中轨止盈 |
| 耗时 | 35秒（20个策略） |

### 评分结果

| 等级 | 策略 | 交易数 | PnL | 胜率 | 盈亏比 | 回撤 | 总分 |
|:----:|:----|:----:|:----:|:----:|:----:|:----:|:----:|
| **A** | **mfi_bb_m30_optimized** | 33 | **+$66.60** | **57.6%** | 1.38 | 0.51% | **70** |
| **A** | **rsi_grading_m30_upgraded** | 22 | **+$117.79** | **54.5%** | **1.55** | 1.57% | **70** |
| B | m30_bb_deepreturn | 108 | +$213.95 | 54.6% | 1.32 | 1.25% | 68 |
| B | mfi_bb_m30 | 131 | +$198.33 | 54.2% | 1.30 | 1.39% | 68 |
| B | m30_bb_deepreturn_optimized | 157 | +$92.80 | 51.0% | 1.09 | 1.50% | 66 |
| B | momentum_pulse_pro | 102 | +$63.83 | 53.9% | 1.09 | 1.64% | 66 |
| B | rsi_grading_m30 | 28 | +$72.37 | 46.4% | 1.46 | 0.82% | 66 |

### 关键发现

1. **所有盈利策略均为反转类（REVERSAL）** — 7个盈利策略全是 M30 反转策略，说明 XAUUSD 在 M30 周期上反转效应显著
2. **趋势类全军覆没** — 8个趋势策略仅 momentum_pulse_pro 微利，其余全亏，亏损总额 >$2,700
3. **突破策略最差** — h1_breakout 954笔亏$1,086，h1_vol_return 2,212笔亏$2,915
4. **评分+ML全部失败** — entry_score_pro、multi_confluence_quant、viprasol_sniper、xaubot_backup 全部亏损
5. **版本演进对比**：
   - mfi_bb_m30 → optimized：胜率 54.2%→57.6%，PnL +$198→+$67（交易量减少，质量提升）
   - rsi_grading_m30 → upgraded：胜率 46.4%→54.5%，PnL +$72→+$118（明显改善）
   - m30_bb_deepreturn → optimized：PnL +$214→+$93（优化版反而下降，需检查出场逻辑）
6. **无法回测策略**：bakome(需Silver Bullet实时检测)、stoch_trend_h1(需M15缓存)、xaubot(ML模型依赖)

### 收益率分析

按 $10,000 本金 0.01 手计算：

| 策略 | 3个月PnL | 收益率 | 年化估算 |
|:----|:--------:|:-----:|:--------:|
| m30_bb_deepreturn | +$213.95 | 2.14% | 8.6% |
| rsi_grading_m30_upgraded | +$117.79 | 1.18% | 4.7% |
| mfi_bb_m30_optimized | +$66.60 | 0.67% | 2.7% |

**注：** 实际账户 $228，若使用0.1手（10倍杠杆），收益率可放大10倍，但风险也同步放大。

### 组合 Captain 推荐

```
1. mfi_bb_m30_optimized  (Score=70)  — 胜率最高57.6%, 回撤0.51%, 最稳健
2. rsi_grading_m30_upgraded (Score=70) — 盈亏比1.55, 盈利$117
3. m30_bb_deepreturn     (Score=68)  — 盈利$213, 交易量大, 统计显著
```