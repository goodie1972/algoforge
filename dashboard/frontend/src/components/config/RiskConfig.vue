    ...
    <n-grid-item>
      <n-space vertical size="medium">
        ...
        <n-form-item label="全局已实现亏损上限 (%)">
          ...
          <template #feedback>超过此比例后所有策略暂停开仓，适用于 balance-based 已实现亏损</template>
        </n-form-item>

        <n-divider title-position="left">浮动亏损 (单策略)</n-divider>
        <n-form-item label="警告线 (%)">
          <n-input-number :value="params.floating_loss_warn_pct" :min="1" :max="50" :step="0.5"
            @update:value="(v: any) => v && update({ floating_loss_warn_pct: v })" style="width:100%;" />
          <template #feedback>仅记录日志，不阻断</template>
        </n-form-item>
        <n-form-item label="阻断线 (%)">
          <n-input-number :value="params.floating_loss_block_pct" :min="1" :max="50" :step="0.5"
            @update:value="(v: any) => v && update({ floating_loss_block_pct: v })" style="width:100%;" />
        ...
        <n-form-item label="亏损上限 (%)">
          <n-input-number :value="params.per_strategy_realized_loss_pct" :min="1" :max="50" :step="0.5"
            @update:value="(v: any) => v && update({ per_strategy_realized_loss_pct: v })" style="width:100%;" />
          <template #feedback>单策略累计已实现亏损超过此比例时触发阻断</template>
        </n-form-item>
        <n-form-item label="阻断时长 (小时)">
          <n-input-number :value="params.per_strategy_loss_block_hours" :min="1" :max="72"
            @update:value="(v: any) => v && update({ per_strategy_loss_block_hours: v })" style="width:100%;" />
        ...
        <n-form-item label="窗口内最大出场次数">
          <n-input-number :value="params.max_rapid_exits" :min="1" :max="20"
            @update:value="(v: any) => v && update({ max_rapid_exits: v })" style="width:100%;" />
        </n-form-item>
        <n-form-item label="检测窗口 (秒)">
          <n-input-number :value="params.rapid_exit_window_seconds" :min="60" :max="3600" :step="60"
            @update:value="(v: any) => v && update({ rapid_exit_window_seconds: v })" style="width:100%;" />
          <template #feedback>{{ Math.round(params.rapid_exit_window_seconds / 60) }} 分钟内出场超过上限则触发</template>
        </n-form-item>
        ...
        <n-form-item label="锁自动过期 (分钟)">
          <n-input-number :value="params.safety_lock_timeout_minutes" :min="10" :max="1440" :step="5"
            @update:value="(v: any) => v && update({ safety_lock_timeout_minutes: v })" style="width:100%;" />
