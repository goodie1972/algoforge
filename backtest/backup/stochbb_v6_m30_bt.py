"""
StochBB v6 — M30 单维度优化
===========================
基于 v5: M30 base (ADX+EMA21) 已经接近不亏 (-$22.60, 盈亏比 0.81).
4 个独立单维度微调 (一次只改一个变量, 看哪个影响最大):

  E1 base:     v5 base,  不变
  E2 dist:     加距离门禁 |close - MA| > 0.5 ATR
  E3 sl:       SL 从 1 ATR 提到 1.5 ATR
  E4 tp:       加固定 TP 1.5 ATR (在 main 逻辑之外)
  E5 time:     加时间止损 24 根 (M30 上 = 12 小时)

⚠️ 数据仅 2026-03-24 → 2026-06-19 (88 天, 2845 根), 样本很小.
"""
import os
import sys
...
def run_backtest(candles, ma_type, ma_period, regime_method,
                 ...
                 dist_gate=0.0,           # 距离门禁 (0=关闭)
                 fixed_tp_atr=None,       # 固定 TP (None=无)
                 time_stop_bars=0,        # 时间止损 (0=无)
    ...
    for i in range(251, n):
        ...
        # ── 出场 ──
            ...
            # 2. 固定 TP
            if exit_reason is None and fixed_tp_atr is not None and pnl_pts >= entry_info['atr'] * fixed_tp_atr:
                exit_reason = "fixed_tp"
                if position == "LONG":
                    exit_p = entry_info['price'] + entry_info['atr'] * fixed_tp_atr
                else:
                    exit_p = entry_info['price'] - entry_info['atr'] * fixed_tp_atr

            # 3. 时间止损
            if exit_reason is None and time_stop_bars > 0 and (i - entry_info['idx']) >= time_stop_bars:
...
def main():
    print("=" * 90)
    print("  StochBB v6 — M30 单维度优化")
    print("=" * 90)
    print("  基线: v5 M30-base = P/L -$22.60, 盈亏比 0.81, 胜率 29.9%")
    print("  数据: M30 2026-03-24 → 2026-06-19, 2845 根 (88 天)")
    print()

    candles = load_ohlcv("M30")

    configs = [
        ("E1 base",      {"sl_atr": 1.0, "dist_gate": 0.0, "fixed_tp_atr": None, "time_stop_bars": 0}),
        ("E2 dist=0.5",  {"sl_atr": 1.0, "dist_gate": 0.5, "fixed_tp_atr": None, "time_stop_bars": 0}),
        ("E3 sl=1.5",    {"sl_atr": 1.5, "dist_gate": 0.0, "fixed_tp_atr": None, "time_stop_bars": 0}),
        ("E4 tp=1.5",    {"sl_atr": 1.0, "dist_gate": 0.0, "fixed_tp_atr": 1.5, "time_stop_bars": 0}),
        ("E5 time=24",   {"sl_atr": 1.0, "dist_gate": 0.0, "fixed_tp_atr": None, "time_stop_bars": 24}),
    ]
    ...
    print("  对比表 (按 P/L 降序)")
    print("=" * 130)
    print(f"  {'配置':<12} {'交易':>5} {'胜':>4} {'负':>4} {'胜率':>7} {'总盈亏':>10} {'均单':>8} {'最大回撤':>10} {'盈亏比':>7} {'L':>4} {'S':>4} {'均K线':>7}")
    print("  " + "-" * 130)
    ...
    # 单维度影响分析
    print(f"\n  维度影响 (vs base):")
    base_pnl = next(r['total_pnl'] for r in results if r['name'] == "E1 base")
    for r in results:
        if r['name'] == "E1 base":
            continue
        delta = r['total_pnl'] - base_pnl
        sign = "↑" if delta > 0 else "↓"
        print(f"    {r['name']:<12}: {sign} ${delta:+.2f} (从 {base_pnl:+.2f} → {r['total_pnl']:+.2f})")
    print()
