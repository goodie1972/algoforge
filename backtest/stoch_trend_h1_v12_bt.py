"""
stoch_trend_h1_upgraded v12 KDJ 周期策略 回测
  入场: ADX>25 + KDJ交叉 + K极值(<20/>80) + BBI方向
  出场: 等 K/D 到反向极值, 然后 KDJ 反向交叉
  无硬止损, 无 BBI 反转出场
"""
import sqlite3
import numpy as np
...
def run_sim_v12(df):
    ...
    for i in range(50, n):
        ...
        if pos_dir is not None:
            ...
            entry_d = trail.get('entry_d', 50)
            ex = False
            reason = None
            if is_buy and entry_k < K_EXTREME_BUY:
                # BUY 等 K>80 且 D>80, 然后 KDJ 死叉
                if k_curr > K_EXTREME_SELL and d_curr > K_EXTREME_SELL and cross_down:
                    ex, reason = True, "BUY KDJ周期"
            elif (not is_buy) and entry_k > K_EXTREME_SELL:
                # SELL 等 K<20 且 D<20, 然后 KDJ 金叉
                if k_curr < K_EXTREME_BUY and d_curr < K_EXTREME_BUY and cross_up:
                    ex, reason = True, "SELL KDJ周期"
            if ex:
                ...
                trades.append({"dir": pos_dir, "ep": ep, "ex": cl, "pnl": pnl,
                                ...
                                "entry_k": entry_k, "entry_d": entry_d})
                pos_dir = None
                trail = {}
        ...
        if pos_dir is None:
            ...
            if adx > 25:
                # 2 & 3. KDJ 交叉 + K 极值
                if cross_up and k_curr < K_EXTREME_BUY and cl > bbi:
                    pos_dir, ep, ei = "BUY", cl, i
                    trail = {"entry_k": k_curr, "entry_d": d_curr, "cross_time": int(df['timestamp'].iloc[i])}
                elif cross_down and k_curr > K_EXTREME_SELL and cl < bbi:
...
def main():
    print("=" * 80)
    print("  stoch_trend_h1_upgraded v12 KDJ 周期策略")
    print("=" * 80)
    print("  入场: ADX>25 + KDJ 交叉 + K 极值(K<20/>80) + BBI 方向")
    print("  出场: 等 K/D 到反向极值, 然后 KDJ 反向交叉")
    print("  无硬止损, 无 BBI 反转出场")
    print("=" * 80)
    ...
    print(stats(trades, "v12 KDJ周期"))

    # 详细列出每笔交易
    if trades:
        print("\n  [每笔交易明细]")
        print(f"  {'方向':4s} {'入场K':>6s} {'入场D':>6s} {'入场价':>8s} {'离场K':>6s} {'离场D':>6s} {'离场价':>8s} {'持仓bars':>8s} {'盈亏':>8s} {'原因':12s}")
        for t in trades:
            ek = t.get('entry_k', 0)
            ed = t.get('entry_d', 0)
            xk = t.get('k_at_exit', 0)
            xd = t.get('d_at_exit', 0)
            from datetime import datetime as dt
            entry_dt = dt.fromtimestamp(t.get('entry_ts', 0)).strftime('%Y-%m-%d')
            print(f"  {t['dir']:4s} {ek:6.1f} {ed:6.1f} {t['ep']:8.2f} {xk:6.1f} {xd:6.1f} {t['ex']:8.2f} {t['bars']:8d} {t['pnl']:>+8.2f} {t['reason']:12s}  ({entry_dt})")
